import React from 'react';
import Link from 'next/link';

export default function ArticlePage() {
    return (
        <main style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem', fontFamily: 'sans-serif', lineHeight: '1.8', color: '#2d3748' }}>
            <header style={{ marginBottom: '2rem', borderBottom: '1px solid #e2e8f0', paddingBottom: '1rem' }}>
                <h1 style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>オセロ初心者必見！勝率を上げる3つのコツ</h1>
                <p style={{ color: '#718096' }}>2025年12月2日</p>
            </header>

            <section style={{ marginBottom: '2rem' }}>
                <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem', borderLeft: '4px solid #3182ce', paddingLeft: '0.5rem' }}>1. 四隅（角）を取る</h2>
                <p>
                    オセロにおいて最も重要な場所は、盤面の四隅（角）です。
                    角に置かれた石は、決して相手にひっくり返されることがありません。
                    角を取ることで、そこを起点に周囲の石を確定石（ひっくり返されない石）にしていくことができます。
                    逆に、相手に角を取らせないように、角の隣（XやCと呼ばれるマス）には安易に石を置かないように注意しましょう。
                </p>
            </section>

            <section style={{ marginBottom: '2rem' }}>
                <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem', borderLeft: '4px solid #3182ce', paddingLeft: '0.5rem' }}>2. 序盤は石を取りすぎない</h2>
                <p>
                    初心者がやりがちな間違いの一つが、序盤からたくさん石を取ってしまうことです。
                    オセロは最終的に石が多い方が勝ちですが、序盤に石を取りすぎると、自分の打てる場所が減ってしまい、相手に有利な場所（角など）を打たされてしまう「手詰まり」の状態になりやすくなります。
                    序盤から中盤にかけては、あえて石を少なく保ち、相手に打つ場所を制限させる「中割り」などのテクニックが有効です。
                </p>
            </section>

            <section style={{ marginBottom: '2rem' }}>
                <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem', borderLeft: '4px solid #3182ce', paddingLeft: '0.5rem' }}>3. 相手の打てる場所を減らす</h2>
                <p>
                    オセロは「相手が良い手を打てないようにする」ゲームでもあります。
                    自分が打つことで、相手が次に打てる場所がどう変わるかを意識しましょう。
                    例えば、相手が角の隣に打たざるを得ないような状況を作れば、次のターンで自分が角を取れるチャンスが生まれます。
                    常に数手先を読み、相手をコントロールする意識を持つことが上達への近道です。
                </p>
            </section>

            <div style={{ marginTop: '3rem', paddingTop: '2rem', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between' }}>
                <Link href="/blog" style={{ color: '#3182ce', textDecoration: 'underline' }}>&larr; コラム一覧へ戻る</Link>
                <Link href="/othello" style={{ color: '#3182ce', textDecoration: 'underline' }}>オセロをプレイする &rarr;</Link>
            </div>
        </main>
    );
}
