export const RELEASES = [
    {
        version: '2.13.0',
        date: '2026.02.23',
        title: 'スマホアプリ化 (PWA) & オフライン対応',
        categories: ['Update', 'System', 'Feature'],
        content: [
            {
                head: 'スマホのホーム画面に追加可能に (PWA対応)',
                text: 'Asobi Loungeをスマートフォンやタブレットのホーム画面に追加し、ネイティブアプリのように全画面で起動できるようになりました。iOS (Safari) および Android (Chrome) の両方に対応しています。'
            },
            {
                head: 'オフラインプレイ対応',
                text: '一度開いたトップページや一人用ゲーム（将棋AI、クリッカー、数独など）は端末に保存され、通信環境がないオフライン状態でも引き続き遊べるようになりました（※オンラインマルチプレイ機能は利用不可となります）。'
            },
            {
                head: 'オフライン時の誤操作防止',
                text: 'オフライン状態で誤って「オンライン対戦」や「ルーム対戦」を選んでしまった場合に、エラーページへ遷移するのを防ぐ専用のアラート機能を追加しました。'
            }
        ]
    },
    {
        version: '2.12.0',
        date: '2026.02.15',
        title: 'Typing Battle (タイピングバトル) 正式リリース',
        categories: ['New Game', 'Feature'],
        content: [
            {
                head: '新作「Typing Battle」登場！',
                text: '迫りくる単語をタイプして相手陣地に押し込む、白熱の対戦型タイピングゲーム！正確性とスピードが勝負の鍵を握ります。「ん」の入力も柔軟に対応し、快適なタイピング体験を提供します。'
            },
            {
                head: '練習モード搭載',
                text: '1人でじっくり練習できるモードも完備。WPM（1分間の入力単語数）や正確率を計測して、タイピングスキルを向上させましょう。'
            },
            {
                head: 'PC専用コンテンツ',
                text: 'キーボード操作に特化したゲーム性のため、PC環境でのみプレイ可能です。大画面での快適なタイピングをお楽しみください。'
            }
        ]
    },
    {
        version: '2.11.0',
        date: '2026.02.12',
        title: 'Quiz Battle (クイズバトル) 正式リリース',
        categories: ['New Game', 'Feature'],
        content: [
            {
                head: '新作「Quiz Battle」リリース',
                text: '最大6人で遊べる早押しクイズゲームが登場！「ゲーム」「アニメ」「歴史」「科学」など主要ジャンルに加え、750問以上の問題を収録。知識とスピードで勝利を掴みましょう！'
            },
            {
                head: '快適なプレイ環境',
                text: '試合終了後にそのまま「ルームに戻る」機能を追加し、連続プレイが快適になりました。また、ローディング画面や進捗表示（Q 1/10）などのUIも強化されています。'
            },
            {
                head: '練習モード搭載',
                text: '1人でじっくり遊べる「練習モード」を搭載。苦手なジャンルを選んで特訓したり、スキマ時間に知識を深めることができます。'
            }
        ]
    },
    {
        version: '2.10.0',
        date: '2026.02.11',
        title: '数独バトル機能強化 & UI改善',
        categories: ['Update', 'Game', 'Feature'],
        content: [
            {
                head: '数独バトル (Sudoku) 機能強化',
                text: 'ランダムマッチングの不具合を修正し、スムーズに対戦相手が見つかるようになりました。また、対戦中に相手の盤面状況をリアルタイムで確認できる機能を追加し、白熱したバトルを楽しめるようになりました。'
            },
            {
                head: '視認性と操作性の向上',
                text: '盤面のレイアウトを刷新し、常に正方形で表示されるよう調整しました。スマートフォンでの表示も最適化し、縦並びレイアウトで快適にプレイできます。進捗率の計算ロジックも改善し、正確な達成度が表示されるようになりました。'
            }
        ]
    },
    {
        version: '2.9.0',
        date: '2026.01.30',
        title: 'Rainbow 正式リリース & モバイル対応',
        categories: ['New Game', 'Feature'],
        content: [
            {
                head: '新作カードゲーム「Rainbow」',
                text: '色と数字を合わせて手札を減らす定番カードゲームが新登場！3x3のクロスレイアウトで、誰のターンかが一目で分かります。'
            },
            {
                head: '快適な操作性',
                text: '手札のドラッグスクロールに対応し、大量のカードもスマホで快適に操作可能。「色順」「数字順」のソート機能も搭載しました。'
            },
            {
                head: '高度な戦略',
                text: '複数枚出しや、ドローカードへのカウンタープレイなど、戦略的なルールを実装。'
            }
        ]
    },
    {
        version: '2.8.0',
        date: '2026.01.29',
        title: 'マルバツゲーム リリース & 機能追加',
        categories: ['New Game', 'Update'],
        content: [
            {
                head: 'マルバツゲーム (Tic-Tac-Toe) リリース',
                text: 'シンプルながら奥深い「マルバツゲーム」が登場！3つ目までは通常通り、4つ目を置くと一番古いコマが消える「無限」ルールを採用し、終わりのない攻防を楽しめます。'
            },
            {
                head: 'マルチプレイ & チャット機能',
                text: '世界中のプレイヤーとランダムマッチ、または友達とルームIDで対戦が可能。対戦中はリアルタイムチャットでコミュニケーションが取れます。'
            },
            {
                head: 'AI対戦モード',
                text: '練習用にAI対戦モードも搭載。空き時間にサクッと遊べます。'
            }
        ]
    },
    {
        version: '2.7.0',
        date: '2026.01.28',
        title: '絵しりとり リリース & UI改善',
        categories: ['New Game', 'Drawing'],
        content: [
            {
                head: '新作「絵しりとり」リリース',
                text: '絵を描いて言葉を繋げる「絵しりとり」が登場！前の人の絵を予想して答えを入力し、次の人に新しい絵のバトンを渡しましょう。'
            },
            {
                head: '高機能キャンバス & マルチプレイ',
                text: 'リアルタイムで同期される軽量キャンバスを搭載。ペン、消しゴム、ズームなどの基本機能に加え、レイヤー構造を排除したシンプルなUIでお絵かきに集中できます。友達とのルーム対戦にも対応。'
            },
            {
                head: 'モバイル最適化',
                text: 'スマートフォンでも快適に遊べるよう、入力フォームや描画ツールを最適化しました。外出先でも手軽に絵しりとりを楽しめます。'
            }
        ]
    },
    {
        version: '2.6.0',
        date: '2026.01.26',
        title: 'Orbit Star リリース',
        categories: ['New Game', 'Puzzle'],
        content: [
            {
                head: '新作パズル Orbit Star',
                text: '360°全方位から惑星を投入し、合体させて進化させる物理パズルゲーム「Orbit Star」をリリースしました。同じ惑星をぶつけて進化させ、ハイスコアを目指しましょう！'
            }
        ]
    },
    {
        version: '2.5.0',
        date: '2026.01.21',
        title: 'Trash Factory リリース & ビリヤード物理改善',
        categories: ['New Game', 'Update'],
        content: [
            {
                head: 'Trash Factory リリース',
                text: 'ゴミを圧縮して資源に変えるシミュレーションゲーム「Trash Factory」をリリースしました。PC専用の本格的な生産管理ゲームです。'
            },
            {
                head: 'ビリヤード物理挙動改善',
                text: 'ポケットの吸い込み判定を導入し、ボールがより自然にポケットに入るように調整しました。'
            }
        ]
    },
    {
        version: '2.4.0',
        date: '2026.01.15',
        title: 'ビリヤード (8-Ball) 正式リリース',
        categories: ['New Game', 'Game', 'Feature'],
        content: [
            {
                head: 'ビリヤード (8-Ball) リリース',
                text: 'リアルな物理演算を搭載した本格的な「8ボール」をリリースしました。Matter.jsによりボールの挙動を忠実に再現しています。'
            },
            {
                head: 'マルチプレイ対応',
                text: '世界中のプレイヤーと対戦できる「ランダムマッチ」に加え、友達と合言葉で遊べる「ルーム対戦」機能を搭載しました。'
            },
            {
                head: 'ソロ練習モード',
                text: '一人でじっくりショットの練習ができるソロモードも完備。物理挙動の確認や、角度の調整など自由にプレイできます。'
            }
        ]
    },
    {
        version: '2.3.0',
        date: '2026.01.14',
        title: 'Global Chat統合 & UI改善',
        categories: ['Update', 'System', 'Feature'],
        content: [
            {
                head: 'Global Chat & サポート統合',
                text: 'チャットボット機能をGlobal Chatに統合し、「サポート」タブとして利用可能にしました。これにより、画面上のボタンが整理され、より快適に操作できるようになりました。'
            },
            {
                head: 'フレンド表示の改善',
                text: 'オンラインフレンドリストに、プロフィールで設定した名前とアイコンが正しく表示されるようになりました。'
            },
            {
                head: 'UI改善',
                text: 'スマートフォンでのチャット表示を最適化し、ボタン配置を調整しました。'
            }
        ]
    },
    {
        version: '2.2.0',
        date: '2026.01.11',
        title: '人狼ゲーム (Werewolf) リリース & 公式Discord開設',
        categories: ['New Game', 'Community', 'Feature'],
        content: [
            {
                head: '人狼ゲーム (Werewolf) リリース',
                text: 'オンラインで遊べる「人狼ゲーム」をリリースしました。配役設定、投票システム、チャット機能など、ブラウザだけで本格的な人狼が楽しめます。友達とルームを作って遊ぶことも、ランダムマッチで対戦することも可能です。'
            },
            {
                head: '公式Discordサーバー開設',
                text: 'Asobi Loungeの公式Discordサーバーを開設しました。開発情報の先行公開や、ユーザー同士の交流、フィードバックの受付を行っています。ヘッダーのアイコンから参加できます。'
            }
        ]
    },
    {
        version: '2.1.0',
        date: '2026.01.09',
        title: 'カードゲームUI改善 & Stock Simulator更新',
        categories: ['Update', 'Game', 'Feature'],
        content: [
            {
                head: 'カードゲーム (Divinduel) UI改善',
                text: 'モバイル版の操作性を大幅に向上させました。手札のスクロール修正、プレイしたカードの視認性向上（サイズ統一・最前面表示）、アクションボタンのレイアウト最適化を行いました。また、PC版のボタンデザインもモダンなスタイルに統一しました。'
            },
            {
                head: 'Stock Simulator プロフィール連携',
                text: 'ランキングやクラウド同期ステータスに表示される名前が、Googleアカウント名からサイト内のプロフィール設定名に変更されました。これにより、他のゲームと同じ名前でランキングに参加できるようになります。'
            }
        ]
    },
    {
        version: '2.0.0',
        date: '2026.01.07',
        title: 'UI全面リニューアル & ゲームリスト拡充',
        categories: ['Update', 'System', 'Game'],
        content: [
            {
                head: 'デザイン刷新',
                text: 'ランディングページおよびログイン後のゲーム一覧（ダッシュボード）のデザインを完全にリニューアル。アニメーションを取り入れ、より美しく使いやすいインターフェースになりました。'
            },
            {
                head: 'ゲーム追加・名称変更',
                text: '「蜂の陣」「バックギャモン」「簡易将棋」など、リストに表示されていなかったゲームを追加し、全23種類のゲームに簡単にアクセスできるようになりました。「どうぶつ将棋」の名称を「ファンタジー将棋」に変更しました。'
            },
            {
                head: 'システム改善',
                text: 'スマートフォンでの表示を最適化し、プロフィールや最新情報へのアクセスを改善しました。'
            }
        ]
    },
    {
        version: '1.6.0',
        date: '2026.01.02',
        title: 'システム＆プロフィール大型アップデート',
        categories: ['System', 'Feature'],
        content: [
            {
                head: 'チャットボット',
                text: 'ゲームプレイ中にチャットボットのアイコンを自動的に非表示にする機能を追加。盤面が見やすくなりました。'
            },
            {
                head: 'プロフィール機能',
                text: 'フレンド機能（申請・承認・ブロック）、詳細な対戦戦績の表示、プロフィール編集機能を追加。'
            }
        ]
    },
    {
        version: '1.5.0',
        date: '2026.01.02',
        title: 'ゲーム機能強化 (蜂の陣・ピアノ・麻雀)',
        categories: ['Game', 'Update'],
        content: [
            {
                head: '蜂の陣 (Honeycomb)',
                text: 'AI対戦モードを復旧し、思考ロジックを強化（Minimax法導入）。'
            },
            {
                head: 'Virtual Piano',
                text: 'サンプラーエンジンを刷新し、よりリアルな音質を実現。ボリュームコントロールとホワイトテーマUIを追加。'
            },
            {
                head: '麻雀 (Mahjong)',
                text: 'オンライン対戦機能を実装。効果音(SE)や役一覧表示を追加し、プレイ感を向上。'
            }
        ]
    },
    {
        version: '1.4.5',
        date: '2026.01.01',
        title: 'お絵かきクイズ (Drawing) 修正',
        categories: ['Fix', 'Game'],
        content: [
            {
                head: 'UI/UX改善',
                text: 'プレイヤー名の表示不具合を修正。ゲーム開始時の挙動を安定化。キャンバスの描画同期ズレを解消。'
            }
        ]
    },
    {
        version: '1.4.0',
        date: '2025.12.25',
        title: 'Civilization Builder 拡張',
        categories: ['Feature', 'Game'],
        content: [
            {
                head: 'ゲームバランス調整',
                text: '製油所（Refinery）の容量ロジックを修正。手動回収の実績解除判定を修正。'
            },
            {
                head: 'UI改善',
                text: '金融（Finance）画面の同期処理を改善。スキルツリーのUI表示バグを修正。'
            }
        ]
    },
    {
        version: '1.2.0',
        date: '2025.12.07',
        title: 'Block Territory (Polyomino) リリース',
        categories: ['New Game', 'Puzzle'],
        content: [
            {
                head: '新作パズルゲーム',
                text: 'テトリスのようなブロックを使って陣地を取り合う「Block Territory」をリリース。AI対戦に対応。'
            }
        ]
    },
    {
        version: '1.0.0',
        date: '2025.12.05',
        title: 'Civilization Builder リリース',
        categories: ['New Game', 'Simulation'],
        content: [
            {
                head: '新作シミュレーション',
                text: '資源管理と文明発展を楽しむクリッカー系ゲーム「Civilization Builder」をリリース。'
            }
        ]
    }
];
